<template>
    <div class="article-page">
        ...
        <StaggeredMenu
          ...
          :colors="['#9EF2B2', '#27FF64']"   :logo-url="logo"
          accent-color="#27FF64"             @menu-open="handleMenuOpen"
  ...
  .fixed-menu-container {
    ...
    height: 100vh;
    z-index: 9999; /* 保证层级最高 */
    pointer-events: none; /* 关闭状态下，鼠标事件穿透给文章内容 */
  }
  
  .fixed-menu-container.is-open {
    pointer-events: auto; /* 展开状态下，接管所有鼠标事件，防止误触底部文章 */
  }
  
  /* 强制菜单的 header 部分（Logo和右侧汉堡按钮）始终可以点击 */
  ...
  .scroll-wrapper {
    ...
    z-index: 10;
    overflow-y: auto;
    padding: 6rem 2rem 4rem 2rem; /* 顶部留出 6rem 给右上角的菜单栏呼吸空间 */
  ...
  .toc-item:hover { color: rgba(255, 255, 255, 0.9); }
  .toc-item.active {
    color: #ffffff;
    font-weight: 500;
  ...
  .toc-item.level-3 { padding-left: 1.5rem; font-size: 0.85rem; }
  ...
  .markdown-body { font-family: sans-serif; color: rgba(255, 255, 255, 0.85); }
  .markdown-body :deep(h2), .markdown-body :deep(h3) {
    color: #ffffff; margin: 2.5rem 0 1.2rem 0; font-weight: 600; scroll-margin-top: 80px; 
  }
  .markdown-body :deep(h2) { font-size: 1.8rem; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 0.5rem; }
  .markdown-body :deep(h3) { font-size: 1.4rem; color: #e0e0e0; }
  .markdown-body :deep(p) { line-height: 1.8; margin: 0 0 1.5rem 0; font-size: 1.05rem; }
  .markdown-body :deep(ul) { margin: 0 0 1.5rem 1.5rem; padding: 0; }
  .markdown-body :deep(li) { line-height: 1.8; margin-bottom: 0.5rem; font-size: 1.05rem; }
  
  .markdown-body :deep(blockquote) {
    border-left: 4px solid #ffffff; margin: 2rem 0; padding: 1rem 1.5rem;
    background: rgba(255, 255, 255, 0.05); border-radius: 0 8px 8px 0;
    font-style: italic; color: rgba(255, 255, 255, 0.7);
  }
  
  .markdown-body :deep(pre) {
    position: relative; background: #0f111a; border: 1px solid rgba(255, 255, 255, 0.08);
    border-radius: 12px; padding: 3.5rem 1.5rem 1.5rem 1.5rem; overflow-x: auto;
    margin: 2rem 0; box-shadow: inset 0 0 20px rgba(0,0,0,0.5);
  }
  ...
  .markdown-body :deep(code) { font-family: 'Courier New', Courier, monospace; font-size: 0.95rem; color: #e2e8f0; }
  
  .markdown-body :deep(.copy-code-btn) {
    position: absolute; top: 0.7rem; right: 1rem; padding: 6px 12px;
    border-radius: 6px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1);
    color: rgba(255,255,255,0.6); font-size: 12px; cursor: pointer; transition: all 0.3s ease; font-family: sans-serif;
  }
  
  .markdown-body :deep(.copy-code-btn):hover { background: rgba(255,255,255,0.15); color: #ffffff; }
  .markdown-body :deep(.copy-code-btn.success) {
    background: rgba(255, 255, 255, 0.2); border-color: #ffffff; color: #ffffff; box-shadow: 0 0 10px rgba(255, 255, 255, 0.2);
  }
  
  /* ================= 返回顶部按钮 ================= */
  .back-to-top {
    position: fixed; right: 2rem; bottom: 2rem; width: 50px; height: 50px;
    border-radius: 50%; background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);
    border: 1px solid rgba(255, 255, 255, 0.1); color: #ffffff; display: flex; justify-content: center; align-items: center;
    cursor: pointer; opacity: 0; transform: translateY(20px); pointer-events: none; transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);
    z-index: 50;
  }
  
  .back-to-top.show { opacity: 1; transform: translateY(0); pointer-events: auto; }
  .back-to-top:hover {
    background: rgba(255, 255, 255, 0.15); border-color: #ffffff; color: #ffffff; box-shadow: 0 0 20px rgba(255, 255, 255, 0.3);
  }
